        ...
        uint64_t            initial_timeline_value)
{
	...
	semaphore_type_create_info.initialValue =
	    type & VK_SEMAPHORE_TYPE_BINARY ? 0 : initial_timeline_value;

	semaphore_create_info.pNext = &semaphore_type_create_info;
	semaphore_create_info.flags = 0;

	vk_check(vkCreateSemaphore(
	    device, &semaphore_create_info, vk_ctx->allocator, &semaphore));

	return semaphore;
}

void vk_semaphore_fini(
    const WenVkContext *vk_ctx, VkDevice device, VkSemaphore semaphore)
{
	vkDestroySemaphore(device, semaphore, vk_ctx->allocator);
}

VkFence
    vk_fence_init(
        const WenVkContext *vk_ctx, VkDevice device, VkFenceCreateFlags flags)
{
	VkFence fence;
	...
	vk_check(
	    vkCreateFence(device, &fence_create_info, vk_ctx->allocator, &fence));

	return fence;
}

void vk_fence_fini(const WenVkContext *vk_ctx, VkDevice device, VkFence fence)
{
	vkDestroyFence(device, fence, vk_ctx->allocator);
}
