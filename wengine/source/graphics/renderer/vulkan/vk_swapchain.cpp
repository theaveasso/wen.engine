...
[[nodiscard]] static WenVkSwapchainSupportDetail
query_support_detail(const WenVkContext* ctx);

static void
swapchain_init(WenVkSwapchainContext* swapchain_ctx, const WenVkContext* ctx);

static void
swapchain_fini(WenVkSwapchainContext* swapchain_ctx, const WenVkContext* ctx);
...
static WenVkSwapchainSupportDetail
query_support_detail(const WenVkContext* ctx) {
    WenVkSwapchainSupportDetail support_detail;
    vk_check(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        ctx->physical_device, ctx->surface, &support_detail.capabilities));
    ...
    vk_check(vkGetPhysicalDeviceSurfacePresentModesKHR(
        ctx->physical_device, ctx->surface, &present_modes_count, nullptr));
...
swapchain_init(WenVkSwapchainContext* swapchain_ctx, const WenVkContext* ctx) {
    ...
    swapchain_ctx->extent =
        swapchain_ctx->support_detail.capabilities.currentExtent;

    VkSurfaceFormatKHR surface_format =
        swapchain_ctx->support_detail.formats[0];
    ...
    swapchain_create_info.preTransform =
        swapchain_ctx->support_detail.capabilities.currentTransform;
    ...
    queue_family_indices.reserve(unique_queue_families.size());
    ...
    for (const auto& image : swapchain_ctx->images) {
        swapchain_ctx->image_views.push_back(vk_image_view_init(
            ctx->logical_device,
            image,
...
swapchain_fini(WenVkSwapchainContext* swapchain_ctx, const WenVkContext* ctx) {
    vkDestroySwapchainKHR(
        ctx->logical_device, swapchain_ctx->swapchain, ctx->allocator);
