...
vk_queue_families_get(VkPhysicalDevice physical_device, VkSurfaceKHR surface) {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(
        physical_device, &family_property_count, nullptr);
    ...
    if (!present_index.has_value() || !graphics_index.has_value()) {
        throw std::runtime_error(
            "failed to find suitable queue for graphics and present queues!");
    }
    ...
    queue_families.transfer = transfer_index.has_value()
                                  ? transfer_index.value()
                                  : graphics_index.value();
    queue_families.compute  = compute_index.has_value() ? compute_index.value()
                                                        : graphics_index.value();
...
vk_logical_device_fini(WenVkContext *vk_ctx, VkDevice device) {
    vkDestroyDevice(device, vk_ctx->allocator);
}
