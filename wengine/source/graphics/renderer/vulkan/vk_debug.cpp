...
[[maybe_unused]] VkBool32 debug_callback(
...
VkResult vk_debugger_init(WenVkDebugger *debugger, VkInstance instance, VkDevice device) {
    ...
    VkDebugUtilsMessengerCreateInfoEXT messenger_create_info = vk_messenger_info_init();
    if (msg_fn != nullptr)
    {
...
void vk_debugger_fini(const WenVkDebugger *debugger, VkInstance instance) {
    auto destroy_messenger_pfn =
             reinterpret_cast<PFN_vkDestroyDebugUtilsMessengerEXT>(
                 vkGetInstanceProcAddr(
                     instance,
                     "vkGetDebugUtilsMessengerEXT"
                 )
             );
    if (destroy_messenger_pfn)
    {
        destroy_messenger_pfn(
            instance,
            debugger->messenger,
            nullptr
        );
...
VkDebugUtilsMessengerCreateInfoEXT vk_messenger_info_init() {
    VkDebugUtilsMessengerCreateInfoEXT
        messenger_create_info = {VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT};
