...
static VkInstance
vk_instance_init(const char *app_name);

void
vk_context_init(
    WenVkContext *vk_ctx, const WenWindow *window, const char *app_name) {
    ...
    vk_ctx->queue_families =
        vk_queue_families_get(vk_ctx->physical_device, vk_ctx->surface);
    vk_ctx->logical_device = vk_logical_device_init(
        vk_ctx->physical_device, &vk_ctx->queue_families);
    vk_ctx->queues =
        vk_device_queues_init(vk_ctx->logical_device, &vk_ctx->queue_families);

    vk_debugger_init(
        &vk_ctx->debugger, vk_ctx->instance, vk_ctx->logical_device);
}

void
vk_context_fini(WenVkContext *vk_ctx) {
    vk_logical_device_fini(vk_ctx, vk_ctx->logical_device);

    vkDestroySurfaceKHR(vk_ctx->instance, vk_ctx->surface, vk_ctx->allocator);

    vk_debugger_fini(&vk_ctx->debugger, vk_ctx->instance);
