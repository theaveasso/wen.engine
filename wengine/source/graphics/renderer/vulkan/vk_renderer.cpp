...
static void
set_renderer_state(VkCommandBuffer command_buffer);
...
void
vk_renderer_fini(WenRendererBackend *) {
...
vk_renderer_resize(
    ...
    vk_renderer.image_extent.height =
        vk_renderer.swapchain_context.extent.height;
}
...
vk_renderer_end_frame(WenRendererBackend *renderer, float dt) {
    ...
    update_current_frame_index();
    WenVkFrameBuffer *current_frame =
        &vk_renderer.frames[vk_renderer.current_frame_index];
    ...
    vk_check(vkResetFences(
        vk_renderer.vk_ctx.logical_device, 1, &current_frame->render_fence));
    ...
    VkImageSubresourceRange range =
        vk_image_subresource_range_init(VK_IMAGE_ASPECT_COLOR_BIT, 1, 1, 0);
...
set_renderer_state(VkCommandBuffer command_buffer) {
    ...
    scissor_extent.width =
        static_cast<uint32_t>(vk_renderer.render_area.scissor_dimension.x);
    scissor_extent.height =
        static_cast<uint32_t>(vk_renderer.render_area.scissor_dimension.y);
